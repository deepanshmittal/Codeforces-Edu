# Submission Link: https://codeforces.com/edu/course/2/lesson/4/2/practice/contest/273278/submission/164122315
...
class SegmentTree:
    ...
    def range_max(self, range_left, range_right):
        MAX = -float("inf")
        range_left += self.tree_size
        range_right += self.tree_size
        while range_left <= range_right:
            if range_left & 1:
                MAX = max(MAX, self.tree[range_left])
                range_left += 1
            if not range_right & 1:
                MAX = max(MAX, self.tree[range_right])
                range_right -= 1
            range_right //= 2
            range_left //= 2
        return MAX
...
for x in range(m):
    ...
    if q[0] == 1:
        ...
        val = q[1]
        lo, hi = q[2], n - 1
        a = st.range_max(lo, hi)
        if a >= val:
            while lo <= hi:
                mid = (lo + hi) // 2
                a = st.range_max(lo, mid)
                if a >= val:
                    hi = mid - 1
                else:
                    lo = mid + 1
